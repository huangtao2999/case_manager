<template >
  <iframe-container :isShowHeading="false">
    <dsw-panel class="dsw-right-content-wrapper" :isShowFooter="true">
      ...
      <dsw-pagination slot="panel-footer" :currentPage="paginateInfo.currentPage" :totalRecords="paginateInfo.total" :recordsPerPage="paginateInfo.pageSize" @dsw-pager-change="getDossierTransform"></dsw-pagination>
    </dsw-panel>
  </iframe-container>
</template >
...
export default {
  ...
  methods: {
    ...
    getDossierTransform ({pageIndex, recordsPerPage} = {pageIndex: this.paginateInfo.currentPage, recordsPerPage: this.paginateInfo.pageSize}) {
      const type = this.type
      const searchCode = this.searchCode

      this.isLoadingForTable = true

      this.$https.jsonp(this.$api.getDossierTransform, {params: {page: pageIndex, limit: recordsPerPage, type, searchCode}}).then((result) => {
    ...
    setColumns () {
      this.columns = [
        ...
        {title: '接收对象', field: 'orgName', width: 150, titleAlign: 'center', columnAlign: 'center', isResize: true, overflowTitle: true},
        {title: '移送原因', field: 'reason', width: 260, titleAlign: 'center', columnAlign: 'center', isResize: true, overflowTitle: true},
        {title: '承办人', field: 'createUserName', width: 150, titleAlign: 'center', columnAlign: 'center', isResize: true, overflowTitle: true},
        {title: '承办单位', field: 'createOrgName', width: 150, titleAlign: 'center', columnAlign: 'center', isResize: true, overflowTitle: true},
        {
          title: '提交时间',
          field: 'createTime',
          formatter: (rowData, rowIndex, pagingIndex, field) => {
